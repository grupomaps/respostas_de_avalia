...
import { Plus, Edit2, Trash2, CheckCircle, XCircle, AlertCircle, Link as LinkIcon, Unlink } from 'lucide-react';
...
export function EmpresasTab() {
  ...
  const handleConnectGoogle = (empresaId: string) => {
    showMessage('error', 'Conectar Google OAuth - Implementar OAuth flow completo');
  };

  const handleDisconnectGoogle = async (empresaId: string) => {
    try {
      const { error } = await supabase
        .from('empresas')
        .update({
      ...
      if (error) throw error;
      showMessage('success', 'Google desconectado com sucesso!');
      loadEmpresas();
    } catch (error: any) {
      console.error('Error disconnecting Google:', error);
      showMessage('error', error.message || 'Erro ao desconectar Google');
    }
  };
              ...
              {empresas.length === 0 ? (
                ...
                empresas.map((empresa) => (
                  <tr key={empresa.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-slate-600 text-sm">{empresa.email_responsavel}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="flex items-center gap-2">
                        {empresa.google_conectado ? (
                          <>
                            ...
                            <button
                              onClick={() => handleDisconnectGoogle(empresa.id)}
                          ...
                          <>
                            ...
                            <button
                              onClick={() => handleConnectGoogle(empresa.id)}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, automacao_ativa: e.target.checked })}
                  className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="automacao_ativa" className="ml-2 text-sm font-medium text-slate-700">
